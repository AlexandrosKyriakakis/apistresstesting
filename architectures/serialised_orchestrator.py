...
def create_workers():
    ...
    for i in range(config.Env().PARALLEL_WORKERS):
        container = client.containers.run(
            ...
            environment={
                ...
                'API_DAILY_HOST': 'ws://daily-worker-' + str(i) + ':9765',
                'API_WEEKLY_HOST': 'ws://weekly-worker-' + str(i) + ':9765',
                'API_MONTHLY_HOST': 'ws://monthly-worker-' + str(i) + ':9765',
            },
            network=network_name,
            command='bash -c "python3 run.py"',
    ...
    for i, (key, country) in enumerate(COUNTRIES.items()):
        ...
        container = client.containers.run(
            ...
            environment={
                ...
                'COUNTRY': country,
                'ARCHITECTURE': ARCHITECTURE_SERIALISED_ORCHESTRATOR,
                'API_DAILY_HOST': 'ws://daily-worker-'
                + str(i % config.Env().PARALLEL_WORKERS)
                + ':9765',
                'API_WEEKLY_HOST': 'ws://weekly-worker-'
                + str(i % config.Env().PARALLEL_WORKERS)
                + ':9765',
                'API_MONTHLY_HOST': 'ws://monthly-worker-'
                + str(i % config.Env().PARALLEL_WORKERS)
                + ':9765',
            },
            network=network_name,
